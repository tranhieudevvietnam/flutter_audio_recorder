...
class FlutterAudioRecorder {
 ...
 late Future _initRecorder;
  ...
  void _responseToRecording(Map<String, Object>? response) {
    ...
    if(response!=null){
      _recording.duration = new Duration(milliseconds: int.parse(response['duration'].toString()));
      _recording.path = response['path']?.toString();
      _recording.audioFormat = _stringToAudioFormat(response['audioFormat'].toString());
      _recording.extension = response['audioFormat']?.toString();
      _recording.metering = new AudioMetering(
          peakPower: double.parse(response['peakPower'].toString()),
          averagePower:double.parse( response['averagePower'].toString()),
          isMeteringEnabled:(response['isMeteringEnabled']??false) as bool);
      _recording.status = _stringToRecordingStatus(response['status'].toString());
    }
